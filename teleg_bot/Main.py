...
TOKEN = "568556775:AAHsW06Q7g6qd-K7FQS3d5XBk4xGTqxArc8"
...
def calibration(bot, update, chat_data):
    ...
    if update.message.text != '/start':
        ...
        if 'climate' not in chat_data:
            ...
            update.message.reply_text('Какой климат вам больше нравится?', reply_markup=markup)

        elif 'type_of_rest' not in chat_data:
            type_of_rest_mak = ReplyKeyboardMarkup(type_of_rest, one_time_keyboard=True)
            update.message.reply_text('Какой вид отдыха вам больше симпатизирует?', reply_markup=type_of_rest_mak)
            ...
            update.message.reply_text('Охарактеризуйте ваше желание таскаться за бумагами для визы.',
                                      reply_markup=visa_mak)

        elif 'children' not in chat_data:
            children_mak = ReplyKeyboardMarkup(children, one_time_keyboard=True)
            update.message.reply_text('Будете ли вы брать в путешествие детей?', reply_markup=children_mak)
...
def main():
    updater = Updater(TOKEN,request_kwargs={'proxy_url':'socks5://103.10.52.83:13455'})
